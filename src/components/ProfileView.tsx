...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'products' | 'account' | 'dashboard' | 'about'>('products');
  ...
  // Display 5 store products
  const displayProducts = products && products.length >= 5 ? products.slice(0, 5) : products;
  ...
  return (
    <div className="pb-28 max-w-lg mx-auto px-3 pt-2">
      {/* Container matching Mockup: Dark Glassmorphic Theme */}
      <div className="relative rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl overflow-hidden">
        ...
        <div className="relative px-4 pb-4 -mt-16 text-center">
          ...
          <div className="relative inline-block mx-auto mb-2">
            ...
            <div className="absolute bottom-1 right-1 bg-teal-400 rounded-full p-1 shadow-md" title="Verified Store Merchant">
              <ShieldCheck className="w-3.5 h-3.5 text-slate-950" />
            </div>
          </div>

          {/* Store Profile Card Header */}
          <div className="p-4 rounded-3xl bg-slate-950/85 backdrop-blur-md border border-slate-800 shadow-xl max-w-md mx-auto">
            ...
            <div className="text-xs text-teal-300 font-mono mt-0.5 font-semibold">
              @AGO_Brand
            </div>

            {/* 1. Store Header Stats: with the person name add 3 stats in a row */}
            <div className="grid grid-cols-3 gap-2 my-3.5 py-3 px-3 rounded-2xl bg-slate-900/95 border border-slate-800 shadow-inner">
              <div className="text-center">
                <div className="text-sm sm:text-base font-black text-white tracking-tight">
                  {isFollowing ? '12.5K' : '12.4K'}
                </div>
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold mt-0.5">
                  FOLLOWERS
                </div>
              </div>
              <div className="text-center border-x border-slate-800">
                <div className="text-sm sm:text-base font-black text-teal-400 tracking-tight">
                  5
                </div>
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold mt-0.5">
                  PRODUCTS
              ...
              <div className="text-center">
                <div className="text-sm sm:text-base font-black text-rose-400 tracking-tight">
                  2.1M
                </div>
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-extrabold mt-0.5">
                  LIKES
            ...
            {/* 2. 3 Action Buttons under the stats: */}
            <div className="grid grid-cols-3 gap-2">
              {/* Follow button - teal/green */}
              <button
                onClick={handleFollowToggle}
                className={`py-2.5 px-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition shadow-lg cursor-pointer ${
                  isFollowing
                    ? 'bg-teal-500/20 text-teal-300 border border-teal-500/50 hover:bg-teal-500/30'
                    : 'bg-gradient-to-r from-teal-400 via-teal-500 to-emerald-500 hover:from-teal-300 hover:to-emerald-400 text-slate-950 shadow-teal-500/25'
                }`}
              >
                {isFollowing ? (
                  <>
              ...
              {/* Send Gift button - yellow/gold with gift icon */}
              <button
                onClick={() => setShowGiftModal(true)}
                className="py-2.5 px-2 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500 hover:from-amber-300 hover:to-yellow-300 text-slate-950 font-bold text-xs flex items-center justify-center gap-1.5 transition shadow-lg shadow-amber-500/20 cursor-pointer"
              >
                <Gift className="w-3.5 h-3.5 text-slate-950" />
                <span>Send Gift</span>
              </button>

              {/* Message button - dark with chat icon */}
              <button
                onClick={() => onChatWithBrand('@AGO_Brand')}
                className="py-2.5 px-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 font-bold text-xs flex items-center justify-center gap-1.5 transition shadow-md cursor-pointer"
              >
          ...
          <div className="mt-4 flex bg-slate-950/80 p-1 rounded-2xl border border-slate-800 max-w-md mx-auto overflow-x-auto no-scrollbar">
            <button
              ...
              className={`flex-1 py-2 px-2 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 whitespace-nowrap cursor-pointer ${
                activeTab === 'products'
                  ? 'bg-gradient-to-r from-teal-500 to-cyan-600 text-slate-950 shadow-md font-extrabold'
          ...
          {/* ================= 3. PRODUCT GRID: SHOW 5 PRODUCTS ================= */}
          {activeTab === 'products' && (
                ...
                >
                  ...
                  <div className="p-2.5 flex-1 flex flex-col justify-between">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-1">
                      <button
                        onClick={() => onChatWithBrand('@AGO_Brand')}
                        className="py-1 px-1 rounded-lg bg-slate-800 text-teal-300 text-[10px] font-bold border border-slate-700 flex items-center justify-center gap-1 cursor-pointer hover:bg-slate-700 transition"
                      >
                      ...
                      <button
                        ...
                        className="py-1 px-1 rounded-lg bg-gradient-to-r from-teal-400 to-cyan-500 hover:from-teal-300 hover:to-cyan-400 text-slate-950 text-[10px] font-extrabold flex items-center justify-center cursor-pointer transition shadow"
                      >
                        Buy
                      </button>
                    </div>
